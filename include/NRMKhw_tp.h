...
extern "C"{
	...
	//STeP-TP	
	inline uint16_t read_chip_id(void);
	int tp_openport(void);
	void tp_closeport(void);
	
	//GPIO
	int tp_gpio_init(void);
	void setouttype(TP_PORT port, uint8_t modecode);
	...
	//UART
	int tp_uart_init(uint8_t portnum, TP_UART_CLK_SRC clksrc);
	TP_UART_CLK_SRC	tp_uart_get_clksrc(uint8_t portnum); //get current clock source
	void tp_uart_set_clksrc(uint8_t portnum, TP_UART_CLK_SRC clksrc); //set current clock source
	int setserial(int portnum, TP_UART_CLK_SRC clkcrs);
