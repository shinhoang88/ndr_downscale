...
int compute()
{
  ...
  Gr_Matrix(joint1p,joint2p,joint3p,joint4p,joint5p,joint6p,joint7p);
	...
	// Check Joint Limits & Compute torque command based on validated applying torque
  for (int i=0; i<7; i++)
  {
    if(enctodeg[i] > jointlimit[i] || enctodeg[0] < -jointlimit[i])
    {
      TargetTor[i] = 0;
    }
    else
    {
      ...
      TargetTor[i] = (int)(((f_torque[i])*1000)/(Mt_cosnt[i]*Mt_Amp[i]*G_ratio[i]));
    }
  }
