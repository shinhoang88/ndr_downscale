...
/*
	...
	<ROS NODE>
	input : cal_torque (based on link torque (not motor torque!!))

	node name:
	downscale_cal_torque -> calculated torque from main controller / unit: Nm
	(gravity compensation is included.)
	*** torque converting will be done in this sub-sytem.


	output >> Actual position , Actual velocities, Actual force , moment

	node name
	downscale_actp -> actual deg for each joint / unit : rad
	downscale_actv -> actual vel for each joint / unit : rad/s
...
double f_torque[7]={0,0,0,0,0,0,0}; // torque applied limitation
...
//************************ GRAVITY COMPENSTATE TORQUE ******88888888888888******
...
int compute()
{
	...
	//Gravity compensation torque; in position mode -> REMOVE
	...
	// torque limit: always add the compute torque with gravity compensation term
	// AND compare with the Individual torque limit
	for (int i=0; i<7; i++)
	{
	...
	// Angle limitation & torque command

	if(enctodeg[0] > 180 || enctodeg[0] < -180)
	{
		TargetTor[0] = 0;
	}
	else
	{
		TargetTor[0] = (int)(((f_torque[0])*1000)/(Mt_cosnt[0]*Mt_Amp[0]*G_ratio[0])); // torque converting : (computed torque*1000) / (motor contstant*motor contnous current)
	}

	if(enctodeg[1] > 110 || enctodeg[1] < -110)
	{
		TargetTor[1] = 0;
	}
	else
	{
		TargetTor[1] =  (int)(((f_torque[1])*1000)/(Mt_cosnt[1]*Mt_Amp[1]*G_ratio[1]));
	}

	if(enctodeg[2] > 180 || enctodeg[2] < -180)
	{
		TargetTor[2] = 0;
	}
	else
	{
		TargetTor[2] =(int)(((f_torque[2])*1000)/(Mt_cosnt[2]*Mt_Amp[2]*G_ratio[2]));
	}

	if(enctodeg[3] > 130 || enctodeg[3] < -130)
	{
		TargetTor[3] = 0;
	}
	else
	{
		TargetTor[3] = (int)(((f_torque[3])*1000)/(Mt_cosnt[3]*Mt_Amp[3]*G_ratio[3]));
	}

	if(enctodeg[4] > 180 || enctodeg[4] < -180)
	{
		TargetTor[4] = 0;
	}
	else
	{
		TargetTor[4] =(int)(((f_torque[4])*1000)/(Mt_cosnt[4]*Mt_Amp[4]*G_ratio[4]));
	}

	if(enctodeg[5] > 119.1 || enctodeg[5] < -119.1)
	{
		TargetTor[5] = 0;
	}
	else
	{
		TargetTor[5] = (int)(((f_torque[5])*1000)/(Mt_cosnt[5]*Mt_Amp[5]*G_ratio[5]));
	}

	if(enctodeg[6] > 180 || enctodeg[6] < -180)
	{
		TargetTor[6] = 0;
	}
	else
	{
		 TargetTor[6] = (int)(((f_torque[6])*1000)/(Mt_cosnt[6]*Mt_Amp[6]*G_ratio[6]));
...
int main(int argc, char **argv)
{
	...
	// For trajectory interpolation break off
	initAxes(); //servo on
